...
public class MyArrayList<E> implements MyList<E> {              // Создаём публичный класс MyArrayList<E>, который реализует интерфейс MyList<E>
    ...
    @Override
    public void add(E value) { // изменения                     // Переопределяем метод add. Метод принимает объект класса Е(Object)
    ...
    @Override
    public void remove(int index) {                             // Переопределяем метод remove. Метод принимает индекс
    ...
    @Override
    public int size() {                                         // Переопределяем метод size
